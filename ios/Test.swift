...
//  authentication
//
//  Created by Emerson Coutinho on 18/01/21.
//
import UIKit
import Braspag3Ds
...
public class Test: NSObject {
  @objc(start:effort:)
  public func start(name: String, effort: NSNumber) -> String {
    print("aqui");
      return "Aqui"
  }
  @objc(callbackEx:callback:)
  func callbackEx(params: [String: Any], callback: RCTResponseSenderBlock){
    let tasks = [
      ["name": "test"]
    ]
  ...
  @objc(startSdkProcess:)
  func startSdkProcess(callback: @escaping RCTResponseSenderBlock) {
...
//                    }
                  ...
                  let card = CardData(number: "4000000000001091", expirationMonth: "12", expirationYear: "2021", cardAlias: "testiano", defaultCard: false)
                  let billData = BillToData(contactName: "jose", phoneNumber: 99999999, email: "teste@gmsil.com", street1: "rua sei la", street2: "rua se la 2", city: "são paulo", state: "sp", zipCode: "00000000", country: "BR")
...
//                    let billData = BillToData(contactName: "", phoneNumber: 0, email: "", street1: "", street2: nil, city: "", state: "", zipCode: "", country: "BR", customerId: "42756522821")
                  ...
                  self.sdk.authenticate(orderData: order,
                                       ...
                                       recurringData: nil,
                                       deviceIpAddress: nil) {
                                          (status, authentication, error) in
  
                                          print(status)
                                          print("Authentication: \(authentication?.debugJsonPrint() ?? "nil")")
                                          print("Deu erro: \(error ?? "não")")
                                          callback([NSNull(), "Authentication: \(authentication?.debugJsonPrint() ?? "nil")"])
                          switch status {
                          case .success:
                              break
                          case .unenrolled:
